...
go run ./cmd/marketplace index --artifacts dist/artifacts.json   # index.json with downloads, signed when MARKETPLACE_SIGNING_KEY is set
```
...
attaches them to a release on this repository named `<plugin>-<tag>`, then generates the index, signs
it and publishes `index.json`, `index.json.sig` and `PUBLIC_KEY` to this repository's GitHub Pages.
